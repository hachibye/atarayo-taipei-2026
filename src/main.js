...
/* 히어로(첫 화면) 내용을 세로 가운데처럼 보이도록 여백을 "한 번만" 계산해 고정합니다.
   flex의 justify-content:center를 쓰면 공연 정보를 펼칠 때 높이가 늘어나면서
   위쪽 글자들이 같이 밀려 올라가 이질감이 생기므로, 여백을 고정값으로 잡아 둡니다. */
function layoutHeroCenter(){
  const hero  = document.querySelector(".hero");
  const inner = document.querySelector(".hero-inner");
  const card  = document.getElementById("info-card");
  if (!hero || !inner) return;
  // 펼쳐진 상태에서는 다시 계산하지 않음 (계산하면 화면이 움직임)
  if (card && card.classList.contains("open")) return;
  ...
  const free = hero.clientHeight - padT - padB - inner.offsetHeight;
  inner.style.marginTop = free > 0 ? Math.round(free / 2) + "px" : "0px";
}

/* 화면 "가로" 크기가 바뀔 때만 다시 계산.
   모바일에서 주소창이 접히며 생기는 세로 높이 변화로는 다시 계산하지 않아 화면이 흔들리지 않음 */
...
function renderSong(song){
  ...
  setKaraokeSourceStatus("loading", "逐字時間：正在尋找開源時間碼…");
...
async function loadKaraokeTiming(song){
  ...
  if (!api || typeof api.load !== "function" || typeof api.alignToLocalLyrics !== "function"){
    setKaraokeSourceStatus("miss", "逐字時間：目前使用本地估算同步");
    return;
  }
  ...
  try {
    timed = await api.load(song, {
      ...
      onStatus: event => {
        ...
        if (event.state === "cache") setKaraokeSourceStatus("loading", "逐字時間：已讀取本機快取，正在校正影片偏移…");
        else if (event.state === "fallback") setKaraokeSourceStatus("fallback", "逐字時間：前順位來源沒有結果，改查 AMLL TTML DB…");
        else if (event.state === "offline") setKaraokeSourceStatus("miss", "逐字時間：離線且沒有快取，使用本地估算同步");
      }
    });
  } catch (error) {
    timed = null;
  ...
  if (!timed){
    setKaraokeSourceStatus("miss", "逐字時間：找不到可用來源，使用本地估算同步");
    return;
  }
  const aligned = api.alignToLocalLyrics(timed, song.lyrics);
  if (!aligned){
    setKaraokeSourceStatus("miss", "逐字時間：來源歌詞與本頁不相符，使用本地估算同步");
    return;
  }
  ...
  setKaraokeSourceStatus("ready", `逐字時間：${cachedHint}（已對齊 ${aligned.matchedLines}/${aligned.totalLines} 行，偏移 ${aligned.offset.toFixed(2)} 秒）`);
  updateLyricsSync(true);
}
...
let lastLyricsW = window.innerWidth, lastLyricsH = window.innerHeight;
window.addEventListener("resize", ()=>{
