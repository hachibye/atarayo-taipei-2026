...
`KARAOKE` 開關可以關閉逐字高亮；關閉後仍保留一般歌詞同步、日文讀音與其他工具列功能，設定同樣會保存。

開啟 `KARAOKE` 時，歌詞頁會依照每句已校正的影片時間，在句內逐字顯示高亮；有前奏或間奏的段落不會把空白時間誤塗成歌詞。切換假名／羅馬拼音時，逐字進度會保留。
...
```js
const CACHE_VERSION = "v139";   // → 改成 "v140"
```
