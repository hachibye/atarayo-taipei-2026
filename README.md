...
歌曲頁面的應援動畫（拍手、揮手、跳躍、轉臂與大合唱）會共用目前歌曲的 BPM 拍長，BPM 資料集中在 `src/data.js` 的 `SONG_BPM` 表格中。日文歌詞的漢字會在上方顯示 ruby 假名讀音；下方工具列的「讀音」會依序切換 `假名`、`Romaji` 與 `假名＋Romaji`，方便同時對照。含大合唱的歌詞行會固定使用金黃色，繁中翻譯則維持一般字幕色。
