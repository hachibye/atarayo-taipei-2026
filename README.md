...
歌曲頁面的應援動畫（拍手、揮手、跳躍、轉臂與大合唱）會共用目前歌曲的 BPM 拍長，BPM 資料集中在 `src/data.js` 的 `SONG_BPM` 表格中。日文歌詞的漢字會在上方顯示 ruby 假名讀音；不熟悉假名時，可從下方工具列切換成 `Romaji`，整行日文（包含平假名、片假名與漢字）都會轉成羅馬拼音。
...
```js
const CACHE_VERSION = "v1.6.7";   // → 改成下一個版本號
```
